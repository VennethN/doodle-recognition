...
\begin{tikzpicture}[
    node distance=1.5cm,
    block/.style={rectangle, draw=blue!50, fill=blue!20, minimum width=3.5cm, minimum height=0.8cm, text centered, font=\small},
    arrow/.style={->, thick}
]
...
\node[right of=res1, node distance=3.5cm, text=blue!60] {Pre-trained};
\node[right of=fc1, node distance=3.5cm, text=green!60] {Custom Classifier};
...
\begin{tikzpicture}[
    node distance=2cm,
    box/.style={rectangle, draw=blue!50, fill=blue!10, minimum width=3cm, minimum height=1cm, text centered, font=\small},
    arrow/.style={->, thick}
]

\node[box] (frontend) {Frontend (HTML5 Canvas)};
\node[box, right of=frontend, node distance=4cm] (api) {Flask API};
\node[box, right of=api, node distance=4cm] (models) {Model Backends};

\node[box, below of=frontend, node distance=1.5cm] (canvas) {Drawing Canvas};
\node[box, below of=api, node distance=1.5cm] (predict) {/api/predict};
\node[box, below of=models, node distance=1.5cm] (resnet) {ResNet18};

\node[box, below of=canvas, node distance=1.2cm] (challenge) {Challenge Mode};
\node[box, below of=predict, node distance=1.2cm] (models_api) {/api/models};
\node[box, below of=resnet, node distance=1.2cm] (similarity) {Similarity};

\draw[arrow] (frontend) -- (api) node[midway, above] {Base64 Image};
\draw[arrow] (api) -- (models) node[midway, above] {Tensor};
\draw[arrow] (models) -- (api) node[midway, below] {Predictions};
\draw[arrow] (api) -- (frontend) node[midway, below] {JSON};
...
\begin{center}
\url{https://github.com/[repository]/doodle-recognition}
\end{center}
