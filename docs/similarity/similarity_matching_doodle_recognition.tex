...
The implementation combines Sobel edge detection preprocessing with three complementary matching methods:
\begin{itemize}
    \item \textbf{Sobel Preprocessing}: Hand-designed edge detection filter (NOT learned like CNN filters)
    \item \textbf{Template Matching}: Normalized correlation-based comparison on edge images
...
\begin{itemize}
    ...
    \item \textbf{Sobel Edge Detection}: Apply hand-designed Sobel filter to extract edges
    \item \textbf{Normalization}: Scale to range [0, 1] for comparison
\end{itemize}

\subsection{Sobel Edge Detection Preprocessing}

Before similarity matching, we apply the \textbf{Sobel filter}---a classical edge detection technique that uses hand-designed kernels (NOT learned like CNN filters):
...
\textbf{Why Sobel helps for doodles:}
\begin{itemize}
    \item Doodles are essentially \textbf{line drawings}---edge information is the primary feature
    \item Sobel emphasizes strokes while reducing background noise
    \item Unlike CNN filters, Sobel kernels are \textbf{fixed mathematical formulas}, not learned from data
\end{itemize}

\textbf{Key distinction from CNNs:} While CNNs \textit{learn} their own optimal edge detectors through backpropagation, Sobel uses predetermined kernel weights designed by humans in the 1960s. This is why classical methods cannot adapt to task-specific patterns.
...
\node[box, text width=3.5cm, align=center, below of=input, fill=orange!20] (sobel) {Sobel Edge Detection\\Hand-designed filter};

\node[box, text width=3.5cm, align=center, below of=sobel] (template) {Template Matching\\Correlation scores};
...
% Arrows
\draw[arrow] (input) -- (sobel);
\draw[arrow] (sobel) -- (template);
\draw[arrow] (sobel) -| +(-2,0) |- (features);
\draw[arrow] (sobel) -| +(2,0) |- (histogram);
...
\begin{itemize}
    ...
    \item \textbf{Apply Sobel edge detection} (hand-designed filter)
...
\begin{itemize}
    ...
    \item \textbf{Preprocessing}: Sobel edge detection (hand-designed filter)
...
\node[text width=3.5cm, align=center] at (4, 0.2) {\small Low (~16\%)};
...
The multi-method OpenCV similarity classifier with Sobel preprocessing achieves:

\begin{itemize}
    \item \textbf{Test Accuracy}: ~16\% (on 500-sample test subset)
    \item \textbf{Method Breakdown}:
    \begin{itemize}
        \item Template matching alone: ~17\%
        \item Multi-method combined: ~14-16\%
        \item Histogram matching alone: ~0.2\% (essentially random for 340 classes)
    \end{itemize}
    \item \textbf{Processing Time}: ~0.5-2 seconds per image
    \item \textbf{Model Size}: ~50MB (templates only)
\end{itemize}

\textbf{Note:} These accuracies are far below the ResNet model (75\%), demonstrating the fundamental limitation of hand-crafted features---even with Sobel edge detection preprocessing.
...
OpenCV similarity matching with Sobel edge detection provides a transparent, training-free alternative to deep learning for doodle recognition. While it doesn't match the accuracy of ResNet, it offers:
...
The use of \textbf{Sobel preprocessing} demonstrates a key insight: even with classical edge detection, hand-crafted features cannot match learned representations. The Sobel filter uses \textit{fixed kernels designed by humans in the 1960s}, while CNNs \textit{learn} their own optimal edge detectors through backpropagation---and often discover patterns similar to Sobel, but tailored to the specific task.

The multi-method approach (Sobel + template + features + histogram) demonstrates how combining complementary techniques can improve robustness, a principle that extends to many computer vision applications.

For production doodle recognition, ResNet remains superior. However, similarity matching with Sobel serves as an excellent baseline and educational tool for understanding the difference between hand-designed and learned features.

\section{References}

\begin{itemize}
    \item Sobel, I. (1968). An Isotropic 3×3 Image Gradient Operator. Stanford AI Project.
    \item OpenCV Template Matching: \url{https://docs.opencv.org/4.x/d4/dc6/tutorial_py_template_matching.html}
    \item OpenCV Sobel Derivatives: \url{https://docs.opencv.org/4.x/d2/d2c/tutorial_sobel_derivatives.html}
