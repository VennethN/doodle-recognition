...
The first layers of ResNet apply \textbf{convolutional filters} (kernels) to detect basic features like edges. To illustrate how convolution works, we use the classical \textbf{Sobel filter} as an example:
...
\node[below] at (6, 0.8) {\footnotesize Sobel filter (example)};
...
\end{tikzpicture}
\caption{Convolution operation illustrated with the classical Sobel filter. Note: ResNet's actual kernels are \textit{learned}, not hand-designed like Sobel.}
...
\textbf{Important Note:} ResNet does \textit{not} actually use the Sobel filter---its kernels are \textbf{learned} during training through backpropagation. However, when researchers visualize the learned filters in early CNN layers, they often find edge-detecting patterns strikingly similar to Sobel. This demonstrates that the network \textit{discovers} these useful features on its own, rather than having them prescribed by humans.
